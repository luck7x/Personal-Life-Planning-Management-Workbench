...
test("周视图展示本周任务和完成记录", async ({ page }) => {
  await page.goto("/");
  ...
  await expect(page.locator("[data-week-day-col]").filter({ hasText: "今天" })).toContainText("完成手机首页样例");
  await expect(page.locator("[data-week-day-col]").filter({ hasText: "今天" })).toContainText("整理导师项目材料");
